...
# Lista para clientes eliminados
clientes_eliminados = []

# Mostrar clientes actuales
print("=== CLIENTES ACTUALES ===")
for cliente in clients:
    print(f"ID: {cliente['id']} - {cliente['nombre_completo']}")

# Pedir ID del cliente a eliminar
id_eliminar = int(input("\nIngrese el ID del cliente a eliminar: "))

# Buscar y mover el cliente
...
if cliente_encontrado:
    # Mover cliente a la lista de eliminados
    clientes_eliminados.append(cliente_encontrado)
    clients.remove(cliente_encontrado)
    print(f"\n✓ Cliente '{cliente_encontrado['nombre_completo']}' movido a clientes eliminados")
else:
    print(f"\n✗ No se encontró ningún cliente con ID {id_eliminar}")

# Mostrar resumen
print(f"\nClientes activos: {len(clients)}")
print(f"Clientes eliminados: {len(clientes_eliminados)}")

# Mostrar clientes eliminados si hay alguno
